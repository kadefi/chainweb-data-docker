...
echo -e "Waiting for chainwebdata_build marker..."
 ...
 if [[ -f /usr/local/bin/chainweb-data ]]; then
   node_await
   chainweb-data server --port 8888 --service-host=$CHAINWEB_HOST --p2p-host=$CHAINWEB_HOST --service-port=$CHAINWEB_SERVICE_PORT --p2p-port=$CHAINWEB_P2P_PORT --dbuser=postgres --dbpass=postgres --dbname=postgres -m
   exit
 fi
