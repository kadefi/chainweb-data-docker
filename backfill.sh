...
check=$(curl -SsL -k -m 15 https://$CHAINWEB_HOST:$CHAINWEB_P2P_PORT/chainweb/0.0/mainnet01/cut | jq .height)
if [[ "$check" == "" ]]; then
  until [[ "$check" != "" ]] ; do
    check=$(curl -SsL -k -m 15 https://$CHAINWEB_HOST:$CHAINWEB_P2P_PORT/chainweb/0.0/mainnet01/cut | jq .height)
...
if [[ -f /tmp/backfill ]]; then
    ...
    chainweb-data fill --service-host=$CHAINWEB_HOST --p2p-host=$CHAINWEB_HOST --service-port=$CHAINWEB_SERVICE_PORT --p2p-port=$CHAINWEB_P2P_PORT --dbuser=postgres --dbpass=postgres --dbname=postgres
...
until [[ "$x" == 1 ]] ; do
  ...
  if [[ "$server_check" == 2 ]]; then
    ...
    chainweb-data fill --service-host=$CHAINWEB_HOST --p2p-host=$CHAINWEB_HOST --service-port=$CHAINWEB_SERVICE_PORT --p2p-port=$CHAINWEB_P2P_PORT --dbuser=postgres --dbpass=postgres --dbname=postgres +RTS -N
    sleep 10
